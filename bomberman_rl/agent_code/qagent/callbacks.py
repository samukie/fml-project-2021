...
def construct_table(feature_size, board_size):
    q_table =  np.random.rand(2*board_size+1, 2*board_size+1,2*board_size+1, len(ACTIONS))
    return q_table
...
def get_observation_and_action(self, game_state):
    ...
    # observation
    min_bomb = get_minimum(current, [bomb[0] for bomb in game_state['bombs']], self.board_size)
    min_player =  get_minimum(current, [agent[3] for agent in game_state['others']], self.board_size)
    min_coin =  get_minimum(current, game_state['coins'], self.board_size)
    observation = [min_bomb,min_player,min_coin]
